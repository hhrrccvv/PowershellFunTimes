...
#Warning message
Write-Host "This script will prepare a set of chosen computers to be imaged, by setting them to boot from the PXE option. `nIt will then allow you to trigger mass reboots or shutdowns with an optional delay."

#Confirms the user wants to proceed, or exits the script
$_continueChoice = Read-Host -Prompt "`nPlease note: you will still have to hit 'ENTER' on the computers to select the network boot. `n`nWould you like to continue? (y/n)"
...
if($_continueChoice -like "*y*"){
    ...
    #Clears the screen of all the verbose output
    cls
    ...
    do{
        ...
        $_powerChoice = Read-Host -Prompt "`nWould you like to restart, shutdown, or leave the computers on? (r/s/o)" #NOT ACCEPTING O???
            ...
            "o" {
                    ...
                    if($_revertChoice -like "*y*"){
                        ...
                        foreach($_item in $_onlineList){
                            ...
                            Write-Progress -Activity "Reverting computers..." -Status $_item -PercentComplete $_percentCompleteB
                        ...
                        #Clear progress bar
                        Write-Progress -Activity "Reverting computers..." -Status "Ready" -Completed
                    }
                    else{
                        #Reminds the user they've made a change
                        Write-Host "`nRemember, until they're imaged, the devices will try to PXE on every boot." -ForegroundColor Red
                    }
                }
            }
        } while ($_powerChoice -ne "r" -and $_powerChoice -ne "s" -and $_powerChoice -ne "o")
}
